...
**When in doubt, classify as state-heavy.** The cost of a false positive is one extra
review pass (~$0.40). The cost of a false negative is -15pp on a hard task.

**Decision point:** Make the classification at task start, before invoking the first skill.
Log the decision: "State-heavy: yes/no — [reason]".

If state-heavy: after implementation and first code review, run a SECOND code review
(see requesting-code-review skill, "Second-Pass Review").
